    ...
    public class CurrentState : ICloneable
    {
        ...
        public float ch3percent
        {
            get
            {
                ...
                try
                {
                    if (Main.comPort.MAV.param.ContainsKey("RC3_MIN"))
                    {
                        return (int)(((ch3out - (float)Main.comPort.MAV.param["RC3_MIN"]) / ((float)Main.comPort.MAV.param["RC3_MAX"] - (float)Main.comPort.MAV.param["RC3_MIN"])) * 100);
                    }
                    else
                    {
                        if (ch3out == 0)
        ...
        public bool connected { get { return (Main.comPort.BaseStream.IsOpen || Main.comPort.logreadmode); } }
        ...
        public string GetNameandUnit(string name)
        {
            ...
            {
                desc = desc.Replace("(dist)", "(" + Main.comPort.MAV.cs.DistanceUnit + ")");
            }
            else if (desc.Contains("(speed)"))
            {
                desc = desc.Replace("(speed)", "(" + Main.comPort.MAV.cs.SpeedUnit + ")");
            }
        ...
        {
            UpdateCurrentSettings(bs, false, Main.comPort);
        }
        /*
